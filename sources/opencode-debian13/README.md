...
Same base as `vibeco-debian13`, plus the `opencode` binary baked into
`/usr/local/bin/opencode` via a patch applied to the staged `coder.sh`.
...
Image=ghcr.io/raven428/container-images/opencode-debian13:latest
