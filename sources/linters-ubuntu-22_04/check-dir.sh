...
/usr/bin/env podman run --rm -i -w "${PATH2CONT}" --name="${CONT_NAME}" \
  --hostname="${CONT_NAME}" -e TERRAFORM_DIR -e PROM_RULES_DIR -e USE_GITIGNORE \
  -e "USE_LINTIGNORE" -v "${DIR2CHECK}:${PATH2CONT}:ro" ${PODMAN_ARGS} \
